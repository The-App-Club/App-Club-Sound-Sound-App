...
function analyserInitialize() {
  ...
  analyser.fftSize = Math.pow(2, parameter.fftSizeLevel + 4);
...
function visualizer(dataArray, bufferLength) {
  ...
  for (let index = 0; index < bufferLength; index++) {
    const red = (index * barHeight) / 20;
    const green = index * 4;
    const blue = barHeight / 2;

    barHeight = dataArray[index];
    canvasDomContext.fillStyle = `rgb(${red},${green},${blue})`;
...
gui.add(controllerInfo, "FFT Size Level", 1, 11, 1).onChange((event) => {
  detectChangeParameter(event, "FFT Size Level");
});
...
function detectChangeParameter(event, keyName) {
  ...
  if (keyName === "FFT Size Level") {
    parameter.fftSizeLevel = event;
  }
